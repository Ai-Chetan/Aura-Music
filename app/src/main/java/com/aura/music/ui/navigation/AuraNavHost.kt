...
import com.aura.music.ui.tour.TourController
import com.aura.music.ui.tour.TourNavigator
import com.aura.music.ui.tour.buildAuraTour
...
object Routes {
    ...
    const val ADD = "add"
    const val ADD_PATTERN = "add?initialUrl={initialUrl}&mode={mode}&query={query}"
    ...
    fun add(mode: Int? = null): String =
        if (mode == null) ADD else "add?initialUrl=&mode=$mode&query="

    /** Add destination pre-filled with a search query and results ready. */
    fun addSearch(query: String): String =
        "add?initialUrl=&mode=0&query=" + Uri.encode(query)
}
...
fun AuraNavHost(
    ...
    // The guided tour: overlay state lives here, above the whole shell, and
    // drives the nav controller through TourNavigator as its steps advance.
    val tour = remember { TourController() }
    val tourNavigator = remember(navController, tour) {
    ...
    val tourSteps = remember { buildAuraTour(tourNavigator) }
    ...
    // Full-screen player has its own chrome; every other destination gets
    // the standard app shell: content + MiniPlayer + tabs.
    val showShell = currentRoute != Routes.NOW_PLAYING

    // DataStore loads async — hold the graph until we know whether this is
    // a first launch (guided tour starts automatically once HOME settles).
    if (onboardingCompleted == null) {
        Box(modifier = modifier.fillMaxSize())
        return
    }

    // First launch: once the splash is out of the way (~1.9s), walk the
    // user through the real UI. Replayed later from Settings → Help.
    LaunchedEffect(onboardingCompleted) {
        if (onboardingCompleted == false && !tour.active) {
            delay(2_100)
            if (!tour.active) tour.start(tourSteps, tourNavigator)
        }
    }
    ...
    CompositionLocalProvider(LocalTour provides tour) {
        Box(modifier = modifier.fillMaxSize()) {
                        ...
                        AnimatedVisibility(
                            ...
                            if (currentTrack != null) {
                                MiniPlayer(
                                    ...
                                    modifier = Modifier.tourAnchor(TourAnchors.MINI_PLAYER)
                                )
                            }
                        }
                        NavigationBar(
                            ...
                            NavigationBarItem(
                                ...
                                label = { Text("Library") },
                                modifier = Modifier.tourAnchor(TourAnchors.NAV_LIBRARY)
                            )
                        }
                    }
                }
                    ...
                    NavHost(
                        ...
                        composable(Routes.SETTINGS) {
                            SettingsScreen(
                                ...
                                onGuideClick = { tour.start(tourSteps, tourNavigator) }
                            )
                        }
                        ...
                        composable(
                            ...
                            val modeArg =
                                backStackEntry.arguments?.getInt("mode")?.takeIf { it in 0..1 }
                            AddScreen(
            ...
            // The tour rides above everything — scrim, cutouts and the
            // finish panel (starter downloads + search) live here.
            GuidedTourOverlay(
                controller = tour,
                onSearchRequest = { query ->
                    tour.end()
                    onboardingViewModel.complete()
                    navController.navigate(Routes.addSearch(query)) {
                        popUpTo(Routes.HOME) { inclusive = false }
                        launchSingleTop = true
                    }
                },
                onDownloadStarters = { tracks ->
                    onboardingViewModel.downloadStarterTracks(tracks)
                    endTourAtHome()
                },
                onFinish = { endTourAtHome() }
            )
        }
    }
}
